import { MessageFlags, type BaseInteraction, type InteractionReplyOptions } from "discord.js";
...
export interface InteractionResponder {
  defer(options?: { visibility?: InteractionVisibility }): Promise<Result<void, InteractionResponseError>>;
  ...
  return {
    async defer(options = {}) {
      ...
      if (!replyable.deferReply) {
        return ErrResult(
          new InteractionResponseError("unsupported_interaction", "Interaction does not support deferred replies."),
        );
      }

      const payload = visibilityPayload(options.visibility ?? "public");
      return callDiscord(() => replyable.deferReply?.(payload), "Failed to defer interaction response.");
    },
    ...
    async send(payload) {
      ...
      if (replyable.deferred) {
        if (!replyable.editReply) {
          return ErrResult(
            new InteractionResponseError("unsupported_interaction", "Deferred interaction does not support editReply."),
          );
        }
        return callDiscord(() => replyable.editReply?.(payload), "Failed to edit deferred interaction response.");
      }

      if (replyable.replied) {
        if (!replyable.followUp) {
          return ErrResult(
            new InteractionResponseError("unsupported_interaction", "Replied interaction does not support followUp."),
          );
        }
        return callDiscord(() => replyable.followUp?.(payload), "Failed to send interaction follow-up.");
      }

      if (!replyable.reply) {
        return ErrResult(
          new InteractionResponseError("unsupported_interaction", "Interaction does not support replies."),
        );
      }
...
export function validateInteractionPayload(
  ...
  if (typeof payload.content === "string" && payload.content.length > 2_000) {
    return ErrResult(
      new InteractionResponseError("invalid_payload", "Interaction response content must be 2000 characters or fewer."),
    );
  }

  if (Array.isArray(payload.embeds) && payload.embeds.length > 10) {
    return ErrResult(
      new InteractionResponseError("invalid_payload", "Interaction response must contain 10 embeds or fewer."),
    );
  }

  if (Array.isArray(payload.components) && payload.components.length > 5) {
    return ErrResult(
      new InteractionResponseError("invalid_payload", "Interaction response must contain 5 component rows or fewer."),
    );
  }
...
async function callDiscord(
  ...
  try {
    ...
    return ErrResult(new InteractionResponseError(classifyDiscordInteractionError(error), message, error));
  }
}
