...
export default command("modconfig")
  ...
  .subcommand({
    ...
    options: (s) => s.channel("channel", "Text channel for mod logs", { required: true }),
    run: async (c) => {
  ...
  .subcommand({
    name: "restrict-roles",
    description: "Set a role for a restriction type",
    options: (s) =>
      s
  ...
  .subcommand({
    name: "escalation",
    description: "Configure auto-escalation settings",
    options: (s) =>
      s
