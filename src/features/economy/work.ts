...
export async function work(ctx: Ctx, userId: string, config: WorkConfig = DEFAULT_WORK_CONFIG): Promise<WorkResult> {
  if (ctx.cooldowns.isOnCooldown(userId, WORK_COMMAND)) {
    const remaining = ctx.cooldowns.getRemainingMs(userId, WORK_COMMAND);
    throw new WorkError("ON_COOLDOWN", `On cooldown for ${Math.ceil(remaining / 60000)} more minute(s)`);
  }
  ...
  const payout = Math.floor(Math.random() * (config.maxPayout - config.minPayout + 1) + config.minPayout);
