...
export async function handleTicketClose(interaction: ButtonInteraction): Promise<void> {
  ...
  if (!channelId) {
    await interaction.reply({ content: "Invalid close button.", ephemeral: true });
    return;
  }

  const guild = interaction.guild;
  if (!guild) {
    await interaction.reply({ content: "Cannot close ticket outside of a server.", ephemeral: true });
    return;
  }
