...
export class AlchemyError extends Error {
  constructor(
    public readonly code: "MISSING_ITEMS" | "UNKNOWN_ITEM" | "AI_ERROR",
    message: string,
  ) {
    super(message);
...
export async function generateAIName(ingredients: ItemData[]): Promise<string> {
  const systemPrompt =
    "You are a grim, arcane alchemist in a Dark Victorian / Lovecraftian universe (similar to Elden Ring or Bloodborne).";
...
export async function resolveCrucible(
  userId: string,
  item1: ItemData,
  item2: ItemData,
  item3: ItemData,
...
export async function craftInCrucible(
  ...
  Result<{ outputName: string; outputId: string; isNewDiscovery: boolean }, AlchemyError>
> {
  const userRes = await getUser(userId);
  if (userRes.isErr() || !userRes.unwrap())
    return ErrResult(new AlchemyError("MISSING_ITEMS", "User not found"));
  ...
  for (const [id, qty] of Object.entries(req)) {
    if ((inventory[id] || 0) < qty) {
      return ErrResult(
        new AlchemyError("MISSING_ITEMS", `You don't have enough ${id} to perform this synthesis.`),
      );
    }
  }
  ...
  if (!i1 || !i2 || !i3) {
    return ErrResult(
      new AlchemyError("UNKNOWN_ITEM", "One of these items cannot be placed in the crucible."),
    );
  }
