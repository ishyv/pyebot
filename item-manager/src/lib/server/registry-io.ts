...
function extractObjectBlock(source: string, section: keyof ContentMaps, nextSection?: keyof ContentMaps): string {
	...
	for (const [key, value] of Object.entries(entries)) {
		...
		if (!result.success) {
			for (const issue of result.error.issues) {
				issues.push(`${PACK_PATH} $.${section}.${key}${formatZodPath(issue.path).slice(1)}: ${issue.message}`);
			}
			continue;
		}
...
function sourceEntries<T extends { id: string }>(section: Section, entries: Array<[string, T]>): Array<Sourced<T>> {
